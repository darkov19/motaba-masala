...
export function PackagingProfileForm({
    onDirtyChange,
    writeDisabled = false,
    readOnly = false,
}: PackagingProfileFormProps) {
    ...
    const hasContent = useMemo(() => {
        ...
        if (
            typeof watched.pack_mode === "string" &&
            watched.pack_mode.trim() !== ""
        ) {
            return true;
        }
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [activeItems, activeProfiles] = await Promise.all([
                listItems(true),
                listPackagingProfiles(true),
            ]);
            setPackingItems(
                activeItems.filter(
                    item => item.item_type === "PACKING_MATERIAL",
                ),
            );
    ...
    const onFinish = async (values: ProfileFormValues) => {
        ...
        try {
            ...
            const selectedIDs = normalized
                .map(c => c.packing_material_item_id)
                .filter(id => Number.isFinite(id) && id > 0);
            const uniqueIDs = new Set(selectedIDs);
            if (uniqueIDs.size !== selectedIDs.length) {
                message.error(
                    "Duplicate packing material components are not allowed",
                );
                return;
            }
            if (
                normalized.some(
                    c =>
                        !Number.isFinite(c.qty_per_unit) || c.qty_per_unit <= 0,
                )
            ) {
                message.error(
    ...
    return (
        <Space orientation="vertical" size={16} style={{ width: "100%" }}>
            ...
            <div
                className={`packaging-profile-split${readOnly ? " packaging-profile-split--readonly" : ""}`}
            >
                            ...
                            },
                            {
                                ...
                                render: (_, row) => {
                                    ...
                                    const remaining = Math.max(
                                        0,
                                        row.components.length - preview.length,
                                    );
                                    return (
                                        <Space wrap size={[4, 4]}>
                                            {preview.map(component => {
                                                const label =
                                                    packingLabelById.get(
                                                        component.packing_material_item_id,
                                                    ) ||
                                                    `Item #${component.packing_material_item_id}`;
                                                return (
                                                    <Tag
                                                        key={`${row.id}-${component.packing_material_item_id}`}
                                                    >
                                                        {label} x
                                                        {component.qty_per_unit}
                                                    </Tag>
                                                );
                                            })}
                                            {remaining > 0 ? (
