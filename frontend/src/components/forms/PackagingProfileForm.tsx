...
export function PackagingProfileForm({ onDirtyChange, writeDisabled = false }: PackagingProfileFormProps) {
    ...
    const hasContent = useMemo(() => {
        ...
        return Array.isArray(watched.components) && watched.components.some(c => (c?.qty_per_unit || 0) > 0 || (c?.packing_material_item_id || 0) > 0);
    }, [watched]);
            ...
            <Table<PackagingProfile>
                ...
                columns={[
                    ...
                    { title: "Pack Mode", dataIndex: "pack_mode", render: value => <Tag color="geekblue">{value}</Tag> },
                    {
                        title: "Components",
                        key: "components",
                        render: (_, row) => row.components.length,
                    },
                ]}
            />
        </Space>
