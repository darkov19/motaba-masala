...
- Story 2.2E: [Low][TechDebt][Resolved 2026-02-26] Replace deprecated Ant Design `Space direction` prop with `orientation` in auth loading screen. Ref: Story 2.2E, `frontend/src/App.tsx:643`.
- Story 2.2E: [Low][TechDebt][Resolved 2026-02-26] Replace deprecated Ant Design `Space direction` prop with `orientation` in auth login screen. Ref: Story 2.2E, `frontend/src/App.tsx:656`.
- Story 2.3: [High][Bug][Resolved 2026-02-26] Ensured `CreateRecipe` and `UpdateRecipe` rollback transactions on all error paths by replacing shadowed-error rollback guards with explicit commit-flag rollback-safe pattern. Ref: Story 2.3, `internal/infrastructure/db/sqlite_inventory_repository.go`.
- Story 2.3: [High][TechDebt][Resolved 2026-02-26] Added API contract success-path tests for recipe `update` and `list` endpoints to satisfy success+error coverage expectations. Ref: Story 2.3, `cmd/server/api_server_test.go`.
- Story 2.4: [High][TechDebt][Open 2026-02-26] Add missing party API contract tests for update-success, list-success, and explicit operator write-denial mapping. Ref: Story 2.4, `cmd/server/api_server_test.go:549`, `cmd/server/api_server_test.go:589`, `cmd/server/api_server_test.go:607`.
- Story 2.4: [Med][TechDebt][Open 2026-02-26] Add explicit party read-only UI test asserting operator mode hides create/edit controls while list remains accessible. Ref: Story 2.4, `frontend/src/components/forms/__tests__/PartyMasterForm.test.tsx:33`.
