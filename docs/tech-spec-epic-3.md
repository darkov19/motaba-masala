...
- Story 3.4: Fix `GetItemStockBalance` SQL — current dual-JOIN query produces incorrect results (Cartesian product fan-out) when an item has multiple lots AND adjustments; balance is over-counted proportionally. Replace with two independent subqueries. Ref: `internal/infrastructure/db/sqlite_inventory_repository.go:1462`.
- Story 3.4: Add repository integration tests (Task 3.5 gap) — 4 scenarios: happy-path insert, reason_code persisted, lot_id nullable, balance reflects delta. Ref: `internal/infrastructure/db/sqlite_inventory_repository_test.go`.
- Story 3.4: Add HTTP endpoint tests for reconciliation routes (`/inventory/reconciliation/create`, `/inventory/reconciliation/list`, `/inventory/reconciliation/balance`). Ref: `cmd/server/api_server_test.go`.
- Story 3.4: Fix StockReconciliationPage test for API-rejection error banner path; add happy-path submission test. Ref: `frontend/src/components/forms/__tests__/StockReconciliationPage.test.tsx`.
